...
- Run `npm run build`
- Publish `dist` as the static asset folder generated by the Netlify build
- Switch the Nitro server build to the `netlify` preset (Netlify Functions)
- Let Nitro generate the Netlify function routing so `/api/*` and page routes work
...
| `Page not found` on every route including `/` | Wrong publish directory | `publish = "dist"` in `netlify.toml` ✅ |
| `Module not found` during build | Wrong Node version | `NODE_VERSION=20` in `netlify.toml` ✅ |
| `404` on `/api/info` after deploy | API routes weren't wired to the server function | Nitro's Netlify function config handles this automatically ✅ |
